...
- **RESOLVED 2026-07-16: dispatching `subagent_type: prospect-hunter` or `research-analyst` directly DOES grant them their Lusha tools.** The 2026-07-15 test that said otherwise was confounded, Lusha simply wasn't connected in that session; a same-day 2026-07-16 re-test, run after independently confirming Lusha was live (a free `account_usage` call), showed both named types calling Lusha tools directly with no `ToolSearch` and no `general-purpose` wrapper needed (see comments in `.claude/agents/research-analyst.md` and `.claude/agents/prospect-hunter.md`). **Do a cheap liveness check before any batch that needs Lusha**: call `account_usage` first (free); if Lusha isn't connected in the current session, a named agent silently gets its built-in tools only, no error, so a step that should hit Lusha can look like it just quietly skipped. The old `general-purpose` + explicit `ToolSearch` workaround still works as a fallback if a future check shows the direct grant regressing, but is no longer the default path for Prospect Hunter or Research Analyst. **Company Researcher is still the exception**: it deliberately has no Lusha tools in its frontmatter, not because the grant doesn't work, but because routing it through Lusha (wrapper or otherwise) cost more tokens than it saved, see the next bullet and `.claude/agents/company-researcher.md`.
...
- **Run Prospect Hunter's Workflow 0 pre-pull before Company Researcher, once per batch, not per company.** Company Researcher does not have Lusha access (see above) and should not try to find headcount or a named contact via web search either, that's Prospect Hunter's job and it's demonstrably better at it. Dispatch `subagent_type: prospect-hunter` directly for one batched pre-pull covering the whole cohort (confirm Lusha is live with a free `account_usage` call first, see "Lusha Credit Awareness" above), then paste each company's **Given Firmographics** block from the Pre-Pull Report directly into that company's Company Researcher dispatch prompt. Revised pipeline order: Research Analyst → ICP Scorer → Prospect Hunter Workflow 0 (pre-pull) → Company Researcher (Lean or Full) → Prospect Hunter Workflow 1 (verified email enrichment, Tier A/B only) → Copywriter.
